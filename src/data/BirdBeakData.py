...
class BirdBeakDataGenerator(DataGenerator):
    def __init__(self, original_landmarks, seed: int = 0):
        """
        Initialize the bird beak landmark generator using JAX
        Parameters:
        original_landmarks: array with shape (79, 3) containing the original landmark point coordinates
        """
    ...
    def _resample_landmarks(self, num_landmarks):
        ...
        z_coords = self.original_landmarks[:, 2]
        ...
        new_z = self._linear_interpolate(self.normalized_params, z_coords, new_params)
        return jnp.column_stack((new_x, new_y, new_z))
    # def _resample_landmarks(self, num_landmarks):
    #     """Resample points based on specified number of landmarks using JAX"""
    #     # Determine whether to include the end point
    #     is_closed = jnp.all(self.original_landmarks[0] == self.original_landmarks[-1])
    ...
    def generate_data(self, num_landmarks: int, batch_size: int, transform_params: dict = None):
        ...
        resampled = self.original_landmarks
        # normalize resampled landmarks
        # resampled = self._normalize_landmarks(resampled)
        
        # Generate batch
        batch = jax.vmap(self._generate_single, in_axes=(None, 0, None))(resampled, keys, transform_params)
        return batch
    def _normalize_landmarks(self, landmarks):
        """Normalize landmarks to be within 0 to 1 range"""
        return landmarks / self.total_perimeter
    def visualize_samples(self, samples, figsize=(12, 8)):
        ...
        for i in range(batch_size):
            ...
            plt.plot(samples_np[i, :, 0], samples_np[i, :, 1], samples_np[i, :, 2], 'b-o', markersize=2)
            plt.axis('equal')
