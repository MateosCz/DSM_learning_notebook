...
if __name__ == "__main__":
    ...
    in_grid_sz = (16,16)
    sphere_data_generator_XT = ManifoldDataGenerator2D(grid_size=16, manifold_type="sphere", radius=0.8, flatten=True, seed = get_random_int())
    ...
    # sde_3d = Kunita_Flow_SDE_3D_Eulerian_Optimized(k_alpha=1.6, k_sigma=0.4, grid_num=12, grid_range=[-1,1], x0=x0[0], batch_size=128)
    ...
    # trainer = Trainer.SsmTrainer(seed=get_random_int(), landmark_num=128)
        ...
        # model = SphereCTUNO1D(out_co_dim=3, lifting_dim=16, co_dims_fmults=(1, 2, 4, 8), n_modes_per_layer=(16,8,4,2), norm="instance", act="gelu")
        ...
        checkpoint_path = project_root() + '/checkpoints/sphere_model_neuralOp_1D'
        retrain_checkpoint_path = project_root() + '/checkpoints/sphere_model_retrain_neuralOp_1D'
    
        if not os.path.exists(checkpoint_path):
            train_state = trainer.train_state_init(model, lr=1e-2, model_kwargs={'x': jax.random.normal(jrandom.PRNGKey(get_random_int()), x0[0].shape), 't': jnp.array([0]),'object_fn': 'Heng'})
            train_state, train_loss = trainer.train(train_state, sde_3d, sde_solver, sphere_data_generator_X0, train_steps, 22)
            plt.plot(train_loss)
            plt.show()
            # save the model
            ...
            if retrain:
                train_state, train_loss = trainer.train(train_state, sde_3d, sde_solver, sphere_data_generator_X0, retrain_steps, 22)
                plt.plot(train_loss)
                plt.show()
                # save the model
                config = {"dimension": x0[0].shape}
